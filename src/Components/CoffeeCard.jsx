...
const CoffeeCard = ({ coffee, setCoffees, coffees }) => {
  const handleDelete = (_id) => {
    console.log("Deleting Coffee Card from database", _id);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://coffee-auth-server-fxyvf8jg3-arafat-ibn-abdullahs-projects.vercel.app/coffee/${coffee._id}`,
          {
            method: "DELETE",
          }
        )
  ...
  return (
    <div className=" rounded-md bg-[#F5F4F1] flex justify-around items-center py-8 px-10">
      <div>
        <img className="w-[200px] " src={coffee.img} alt="" />
      </div>
      <div>
        <h3>
        ...
        <h3>
          <span className="font-medium">Chef :</span> {coffee.chef}
        </h3>
        <h3>
          <span className="font-medium">Prices:</span> $90.99
        </h3>
      </div>
      <div className="">
        <button className="bg-[#D2B48C] p-3 rounded mb-1">
          <svg
        ...
        <Link to={`updateCoffee/${coffee._id}`}>
          <button className="bg-[#3C393B] p-3 rounded mb-1">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
        ...
        <button
          ...
          className="bg-[#EA4744]  p-3 rounded mb-1"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
